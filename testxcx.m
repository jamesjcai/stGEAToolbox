...
newc(i1)=1; newc(i2)=2;
figure;
scatter(ste.xy(:,1),ste.xy(:,2),15,newc,'filled'); axis ij; colormap lines(3)
...
figure;
scatter3(ste.xy(:,1),ste.xy(:,2),ones(size(ste.xy(:,2))), 5,newc); axis ij; colormap lines(3)
hold on

[i1,i2]=find(MASK3);
newc=zeros(size(c));
newc(i1)=1; newc(i2)=2;
scatter3(ste.xy(:,1),ste.xy(:,2),zeros(size(ste.xy(:,2))), 5,newc,'filled'); axis ij; colormap lines(3)
