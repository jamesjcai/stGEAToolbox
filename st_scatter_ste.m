...
%         end
    ...
    function callback_SHOWRIDGESPOTS(~,~)
        ...
        f=figure('Visible','off');
        subplot(1,2,1);
        scatter(ste.xy(:,1),ste.xy(:,2),15,newc1,'filled'); axis ij; colormap(lines(length(unique(newc1))));
        box on
        subplot(1,2,2);
        scatter(ste.xy(:,1),ste.xy(:,2),15,newc2,'filled'); axis ij; colormap(lines(length(unique(newc2))));
        box on
...
%             end
    ...
    function callback_MULTIVIEW(~,~)        
        ...
        subplot(1,2,1);
        hh1=scatter(xy(:,1),xy(:,2),10,thisc,'filled'); box on; grid on;
        title(ttxt);
        subplot(1,2,2);        
        hh2=gui.i_gscatter3(sce.s,thisc,1,1); box on; 
        title(clable);
    ...
    function callback_MOVEIT(~,~)
        if isempty(get(h1,'ButtonDownFcn'))
            ...
            brush('off'); rotate3d('off'); pan('off');
