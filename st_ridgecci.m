function [T,cx]=st_ridgecci(ste,group,idx1,idx2,K,methodid)
    ...
    pval=ones(N,1); kstat=zeros(N,1);
    numneigb=zeros(N,1); numouter=zeros(N,1);
    lgene=lrdatabase(1:N,1); rgene=lrdatabase(1:N,2);
    avgneigb=zeros(N,1); avgouter=zeros(N,1);
    stdneigb=zeros(N,1); stdouter=zeros(N,1);
